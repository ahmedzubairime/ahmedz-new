            ...
            <section className="bg-slate-50 dark:bg-[#0b1326] py-20 relative overflow-hidden transition-colors">
                ...
                <div className="relative mx-auto max-w-7xl px-6 lg:px-8">
                    <AnimatedSection animation="fade-up">
                        ...
                        <h1 className="mt-3  text-5xl font-extrabold tracking-tight text-slate-900 dark:text-[#dae2fd]" style={{ letterSpacing: "-0.02em" }}>
                            {isAr ? "تواصل معي" : "Let's Connect"}
                        </h1>
                        <p className="mt-4 max-w-2xl  text-lg text-slate-600 dark:text-[#8f9097]">
                    ...
                    <div className="grid gap-6 sm:grid-cols-3">
                        {contactInfo.map((item, idx) => (
                            <div key={idx} className="rounded-xl bg-white dark:bg-[#131b2e] p-8 text-center transition-all hover:bg-slate-50 dark:hover:bg-[#171f33] border border-slate-200 dark:border-[#222a3d] shadow-sm dark:shadow-none hover:shadow-md dark:hover:shadow-none hover:-translate-y-1">
                                ...
                                <p className="mt-4  text-base font-extrabold text-slate-900 dark:text-[#dae2fd]" dir="ltr">{item.label}</p>
                                <p className="mt-1  text-sm text-slate-500 dark:text-[#8f9097]">{item.sub}</p>
                            </div>
                        ))}
                    </div>
                </AnimatedSection>
                ...
                <AnimatedSection className="mx-auto max-w-3xl px-6 lg:px-8" animation="fade-in" delay={0.3}>
                    {sent ? (
                        <div className="rounded-xl bg-slate-50 dark:bg-[#0b1326] p-16 text-center border border-slate-200 dark:border-[#222a3d] shadow-sm dark:shadow-none">
                            ...
                            <h2 className="mt-4  text-2xl font-extrabold text-slate-900 dark:text-[#dae2fd]">
                                {isAr ? "تم إرسال رسالتك بنجاح!" : "Message Sent Successfully!"}
                            </h2>
                            <p className="mt-3  text-sm text-slate-600 dark:text-[#8f9097]">
                        ...
                        <form onSubmit={handleSubmit} className="space-y-8 bg-slate-50 dark:bg-[#0b1326] p-8 md:p-12 rounded-xl border border-slate-200 dark:border-[#222a3d] shadow-sm dark:shadow-none transition-colors">
                            <div className="grid gap-8 sm:grid-cols-2">
                                <div>
                                    <label className=" text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-[#8f9097]">
                                        {isAr ? "الاسم الكامل" : "Full Name"}
                                    </label>
                                    <input
                                        name="name"
                                        required
                                        className="mt-2 w-full border-b-2 border-slate-300 dark:border-[#45474c]/30 bg-transparent pb-3  text-sm text-slate-900 dark:text-[#dae2fd] placeholder:text-slate-400 dark:placeholder:text-[#45474c] focus:border-[#d4af37] dark:focus:border-[#e9c176] focus:outline-none transition-colors"
                            ...
                            <div>
                                ...
                                <textarea
                                    ...
                                    required
                                    rows={5}
                                    className="mt-2 w-full resize-none border-b-2 border-slate-300 dark:border-[#45474c]/30 bg-transparent pb-3  text-sm text-slate-900 dark:text-[#dae2fd] placeholder:text-slate-400 dark:placeholder:text-[#45474c] focus:border-[#d4af37] dark:focus:border-[#e9c176] focus:outline-none transition-colors"
                            ...
                            <button
                                type="submit"
                                disabled={isPending}
                                className="w-full rounded bg-gradient-to-r from-[#d4af37] to-[#997a15] dark:from-[#e9c176] dark:to-[#C5A059] px-8 py-4  text-sm font-bold text-white dark:text-[#0b1326] transition-all hover:shadow-xl dark:hover:shadow-[#e9c176]/20 disabled:opacity-50"
                            >
                                {isPending

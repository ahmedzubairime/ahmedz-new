...
import { saveAboutSeo } from "@/app/actions/about";
...
export function AboutSeoForm({ locale, initialData }: Props) {
    ...
    function onSubmit(data: FormValues) {
        startTransition(async () => {
            try {
                await saveAboutSeo({ ...data, og_image_id: ogImageId });
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="mx-auto max-w-5xl space-y-6 pb-24 relative">
            <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col gap-1 rounded-3xl border-2 border-white/50 bg-white/40 p-6 backdrop-blur-xl shadow-lg shadow-teal-500/5 dark:border-zinc-800/50 dark:bg-zinc-900/40">
                ...
                <p className="mt-1 flex items-center gap-2 text-sm font-medium text-zinc-500">
                    ...
                    {locale === "ar" ? "إعدادات محركات البحث لصفحة من نحن." : "Search engine optimization for the About page."}
                </p>
            </motion.div>
            ...
            <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
                <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="space-y-4 rounded-3xl border-2 border-zinc-200/60 bg-white/80 p-8 shadow-xl backdrop-blur-md dark:border-zinc-800/80 dark:bg-zinc-900/80">
                    ...
                    <PlayfulInput label="Meta Title" dir="ltr" placeholder="About Us — Company Name" {...register("meta_title_en")} error={errors.meta_title_en?.message} />
                    <PlayfulTextarea label="Meta Description" dir="ltr" rows={3} placeholder="A concise description of the about page..." {...register("meta_description_en")} error={errors.meta_description_en?.message} />
                </motion.div>
                ...
                <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-4 rounded-3xl border-2 border-[var(--brand-primary)]/20 bg-white/80 p-8 shadow-xl backdrop-blur-md dark:border-[var(--brand-primary)]/20 dark:bg-zinc-900/80">
                    ...
                    <PlayfulInput label="عنوان الميتا" dir="rtl" placeholder="من نحن — اسم الشركة" {...register("meta_title_ar")} error={errors.meta_title_ar?.message} />
                    <PlayfulTextarea label="وصف الميتا" dir="rtl" rows={3} placeholder="وصف مختصر لصفحة من نحن..." {...register("meta_description_ar")} error={errors.meta_description_ar?.message} />
                </motion.div>
            </div>
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="space-y-4 rounded-3xl border-2 border-zinc-200/60 bg-white/80 p-8 shadow-xl backdrop-blur-md dark:border-zinc-800/80 dark:bg-zinc-900/80">
                ...
                <p className="text-xs text-zinc-400">{locale === "ar" ? "الصورة التي تظهر عند مشاركة رابط صفحة من نحن في شبكات التواصل." : "The image displayed when the About page URL is shared on social networks."}</p>
                {ogImageUrl ? (

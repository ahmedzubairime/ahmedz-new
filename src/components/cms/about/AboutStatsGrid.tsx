...
import { saveAboutStat, deleteAboutStat } from "@/app/actions/about";
...
export function AboutStatsGrid({ locale, stats }: Props) {
    ...
    function onSubmit(data: FormValues) {
        startTransition(async () => {
            try {
                await saveAboutStat({ ...data, numeric_value: parseInt(data.numeric_value) }, editingId || undefined);
    ...
    function handleDelete(id: string) {
        ...
        startTransition(async () => {
            try { await deleteAboutStat(id); toast.success(locale === "ar" ? "تم الحذف" : "Deleted"); }
            catch { toast.error(locale === "ar" ? "فشل" : "Failed"); }
        });
    }
    ...
    return (
        <div className="space-y-6 max-w-7xl mx-auto">
            <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="flex flex-wrap items-center justify-between gap-4 rounded-3xl border-2 border-white/50 bg-white/40 p-6 backdrop-blur-xl shadow-lg shadow-amber-500/5 dark:border-zinc-800/50 dark:bg-zinc-900/40">
                <div>
                    <h1 className="text-3xl font-black text-zinc-900 dark:text-zinc-50 tracking-tight">{locale === "ar" ? "الإحصائيات" : "Statistics & Counters"}</h1>
                    <p className="mt-1 flex items-center gap-2 text-sm font-medium text-zinc-500"><SidebarIcon name="bar-chart-2" className="size-4 text-amber-500" />{locale === "ar" ? "أرقام مثل المشاريع المنجزة، عدد الموظفين." : "Numbers like completed projects, team size."}</p>
                </div>

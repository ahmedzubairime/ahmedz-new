...
import { saveServicesSeo } from "@/app/actions/cms";
...
export function ServicesSeoForm({ locale, initialData }: Props) {
    ...
    useEffect(() => {
        ...
        setOgImageUrl(initialData?.og_image ? `${process.env.NEXT_PUBLIC_SUPABASE_URL}/storage/v1/object/public/${initialData.og_image.bucket}/${initialData.og_image.storage_path}` : null);
    }, [initialData, reset]);

    function onSubmit(data: FormValues) {
        startTransition(async () => {
            try {
                await saveServicesSeo({ ...data, og_image_id: ogImageId });
    ...
    return (
        <form onSubmit={handleSubmit(onSubmit)} className="mx-auto max-w-5xl space-y-6 pb-24 relative">
            <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col gap-1 rounded-3xl border-2 border-white/50 bg-white/40 p-6 backdrop-blur-xl shadow-lg dark:border-zinc-800/50 dark:bg-zinc-900/40">
                <h1 className="text-3xl font-black text-zinc-900 dark:text-zinc-50 tracking-tight">{locale === "ar" ? "إعدادات SEO — الخدمات" : "Services Page SEO"}</h1>
                <p className="mt-1 flex items-center gap-2 text-sm font-medium text-zinc-500">
                    ...
                    {locale === "ar" ? "تحسين ظهور صفحة الخدمات في محركات البحث." : "Optimize how the services page appears in search engines."}
                </p>
            </motion.div>
            ...
            <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
                <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="space-y-4 rounded-3xl border-2 border-zinc-200/60 bg-white/80 p-8 shadow-xl backdrop-blur-md dark:border-zinc-800/80 dark:bg-zinc-900/80">
                    <div className="mb-4 flex items-center justify-between border-b-2 border-zinc-100/50 pb-4 dark:border-zinc-800/50">
                        <span className="text-xl font-bold">English SEO</span>
                        <span className="rounded-lg bg-zinc-100 px-3 py-1 text-xs font-bold font-mono text-zinc-500 dark:bg-zinc-800 dark:text-zinc-400">LTR</span>
                    </div>
                    <PlayfulInput label="Meta Title" dir="ltr" placeholder="Services | Company Name" {...register("meta_title_en")} />
                    <PlayfulTextarea label="Meta Description" dir="ltr" rows={3} placeholder="Discover our comprehensive services..." {...register("meta_description_en")} />
                </motion.div>
                ...
                <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-4 rounded-3xl border-2 border-[var(--brand-primary)]/20 bg-white/80 p-8 shadow-xl backdrop-blur-md dark:border-[var(--brand-primary)]/20 dark:bg-zinc-900/80">
                    <div className="mb-4 flex items-center justify-between border-b-2 border-[var(--brand-primary)]/10 pb-4">
                        <span className="text-xl font-bold">سيو عربي</span>
                        <span className="rounded-lg bg-[var(--brand-primary)]/10 px-3 py-1 text-xs font-bold font-mono text-[var(--brand-primary)]">RTL</span>
                    </div>
                    <PlayfulInput label="عنوان الميتا" dir="rtl" placeholder="الخدمات | اسم الشركة" {...register("meta_title_ar")} />
                    <PlayfulTextarea label="وصف الميتا" dir="rtl" rows={3} placeholder="اكتشف خدماتنا المتكاملة..." {...register("meta_description_ar")} />
                </motion.div>
            </div>

            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="rounded-3xl border-2 border-zinc-200/60 bg-white/80 p-8 shadow-xl backdrop-blur-md dark:border-zinc-800/80 dark:bg-zinc-900/80">
                <div className="mb-4 flex items-center gap-3 border-b-2 border-zinc-100/50 pb-4 dark:border-zinc-800/50">
                    <div className="flex size-10 items-center justify-center rounded-2xl bg-gradient-to-br from-teal-400 to-teal-600 text-white shadow-lg"><SidebarIcon name="image" className="size-5" /></div>
                    <h3 className="text-xl font-bold text-zinc-900 dark:text-zinc-100">OG Image</h3>
                </div>
                {ogImageUrl ? (
                    <div className="relative overflow-hidden rounded-2xl border-2 border-zinc-200 dark:border-zinc-700 aspect-video max-w-md group shadow-md">
                        <img src={ogImageUrl} alt="OG" className="size-full object-cover" />
                        <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-4 backdrop-blur-sm">
                            ...
                            <PlayfulButton type="button" onClick={() => { setOgImageUrl(null); setOgImageId(null); setIsMediaDirty(true); }} className="!bg-rose-600 !text-white px-6">Remove</PlayfulButton>
                        </div>
                    </div>
                ) : (
                    <div onClick={() => setIsImageDialogOpen(true)} className="group flex cursor-pointer flex-col items-center justify-center rounded-2xl border-2 border-dashed border-zinc-300 bg-zinc-50/50 py-10 max-w-md transition-all hover:border-teal-500 dark:border-zinc-700">
                        <SidebarIcon name="image-plus" className="size-6 text-zinc-400 group-hover:text-teal-500" />
                        <span className="mt-3 text-sm font-bold text-zinc-500 group-hover:text-teal-500">{locale === "ar" ? "اختيار صورة OG" : "Select OG Image"}</span>
                    </div>
                )}
            </motion.div>
            ...
            {isImageDialogOpen && (
                <UploadDialog folders={[]} bucket="images" defaultFolderId="all" locale={locale}
                    ...
                    onSuccess={(urls) => { setIsImageDialogOpen(false); if (urls?.[0]) { setOgImageUrl(urls[0]); setIsMediaDirty(true); } }}
                />
            )}
        </form>
    );
